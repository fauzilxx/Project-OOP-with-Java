...
public class RevenueView {
    ...
    private final ObservableList<Revenue.RevenueRecord> revenueData = FXCollections.observableArrayList();
    ...
    private void fetchRevenueData() {
        revenueData.setAll(Revenue.getRevenueHistory());
    }
    ...
    private VBox buildContent() {
        ...
        // 1. Summary Cards
        Revenue.RevenueRecord today = revenueData.get(revenueData.size() - 1);
        Revenue.RevenueRecord yesterday = revenueData.get(revenueData.size() - 2);
        ...
        for (Revenue.RevenueRecord r : revenueData) {
        ...
        TableView<Revenue.RevenueRecord> table = new TableView<>();
        table.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);
        table.setPrefHeight(400);

        TableColumn<Revenue.RevenueRecord, String> dateCol = new TableColumn<>("Tanggal");
        dateCol.setCellValueFactory(c -> new SimpleStringProperty(c.getValue().date.format(DateTimeFormatter.ofPattern("dd MMMM yyyy"))));
        
        TableColumn<Revenue.RevenueRecord, String> revCol = new TableColumn<>("Total Pendapatan");
        revCol.setCellValueFactory(c -> new SimpleStringProperty(formatCurrency(c.getValue().totalRevenue)));
        
        TableColumn<Revenue.RevenueRecord, String> changeCol = new TableColumn<>("Perubahan (%)");
        ...
        ObservableList<Revenue.RevenueRecord> tableData = FXCollections.observableArrayList(revenueData);
    ...
    private VBox createSummaryCard(String title, long amount, double change) {
        ...
        tLbl.setTextFill(Color.web("#64748b"));
        tLbl.setFont(Font.font(14));
        Label aLbl = new Label(formatCurrency(amount));
        aLbl.setTextFill(Color.web("#0f172a"));
        aLbl.setFont(Font.font("System", FontWeight.BOLD, 24));
        ...
        cLbl.setTextFill(Color.web(color));
        cLbl.setFont(Font.font(13));
