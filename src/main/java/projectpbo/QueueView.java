...
public class QueueView {
    ...
    private HBox buildHeader(){
        ...
        Button backBtn = new Button("← Kembali"); backBtn.setStyle(primaryTextButton()); backBtn.setCursor(Cursor.HAND);
    ...
    private VBox buildContent(){
        ...
        TableView<Queue> table = new TableView<>(); table.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY_ALL_COLUMNS); table.setPrefHeight(520);
        ...
        TableColumn<Queue,String> nameCol = new TableColumn<>("Nama"); nameCol.setCellValueFactory(c -> c.getValue().patientNameProperty());
        TableColumn<Queue,String> numberCol = new TableColumn<>("Nomor Pasien"); numberCol.setCellValueFactory(c -> c.getValue().patientNumberProperty());
        TableColumn<Queue,String> complaintCol = new TableColumn<>("Keluhan"); complaintCol.setCellValueFactory(c -> c.getValue().complaintProperty());
        TableColumn<Queue,String> timeCol = new TableColumn<>("Waktu Masuk"); timeCol.setCellValueFactory(c -> c.getValue().arrivalTimeProperty());
        TableColumn<Queue,String> statusCol = new TableColumn<>("Status"); statusCol.setCellValueFactory(c -> c.getValue().statusProperty());
        ...
        SortedList<Queue> sorted = new SortedList<>(filtered); sorted.comparatorProperty().bind(table.comparatorProperty()); table.setItems(sorted);
        ...
        editBtn.setOnAction(e -> {
            ...
            if(sel!=null){
                fName.setText(sel.getPatientName()); fNumber.setText(sel.getPatientNumber()); fComplaint.setText(sel.getComplaint());
        ...
        doneBtn.setOnAction(e -> {
            Queue sel = table.getSelectionModel().getSelectedItem(); if(sel!=null){ sel.statusProperty().set("Selesai"); Queue.update(sel); table.refresh(); }
        });
        ...
        delBtn.setOnAction(e -> { Queue sel = table.getSelectionModel().getSelectedItem(); if(sel!=null && Queue.delete(sel)) masterData.remove(sel); });
    ...
    private void addQueueEntry(TextField fName, TextField fNumber, TextField fComplaint, TextField fTime, ComboBox<String> fStatus){
        ...
        try { arrival = java.time.LocalDateTime.parse(today+" "+timeRaw, java.time.format.DateTimeFormatter.ofPattern("dd MMMM yyyy HH:mm")); } catch(Exception ex){ arrival = null; }
